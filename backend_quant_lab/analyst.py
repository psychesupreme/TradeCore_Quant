...
def detect_institutional_footprint(df, macro_trend="NEUTRAL", market_regime="NORMAL"):
    """
    3-TIER SMC MATRIX: Scans for Liquidity Sweeps, Price Displacement, Volume Spikes, and FVGs.
    Filtered against H4 Macro Trend and Dynamic Market Regime.
    """
    if len(df) < 50: 
    ...
    if "DEAD MARKET" in market_regime:
        vol_multiplier = 1.0  # NANO Scalper Mode (Accepts average volume)
        atr_multiplier = 0.3
        regime_tag = "[NANO]"
        signal_suffix = "_NANO"
    elif "LOW VOLATILITY" in market_regime or "NORMAL" in market_regime:
        vol_multiplier = 1.2  # MICRO Day Trader Mode
        atr_multiplier = 0.5
        regime_tag = "[MICRO]"
        signal_suffix = ""
    else:
        vol_multiplier = 1.5  # MACRO Sniper Mode
        atr_multiplier = 0.8
        regime_tag = "[MACRO]"
        signal_suffix = ""
    ...
    if sweep_low:
        if displacement_up:
            if not volume_surge:
                return "NEUTRAL", 0.0, f"SMC Tracker {regime_tag}: Bullish FVG blocked (Low Volume Fakeout / Vacuum)."
            
            # NANO doesn't require a deep FVG, just displacement. MICRO/MACRO strictly require FVG.
            if fvg_bullish or regime_tag == "[NANO]":
                if macro_trend == "BEARISH":
                    return "NEUTRAL", 0.0, f"SMC Tracker {regime_tag}: Bullish Setup blocked by Bearish H4 Trend."
                return f"BUY{signal_suffix}", 0.96, f"SMC {regime_tag}: Bullish Setup (H4 + Volume Verified)"
    ...
    if sweep_high:
        if displacement_down:
            if not volume_surge:
                return "NEUTRAL", 0.0, f"SMC Tracker {regime_tag}: Bearish FVG blocked (Low Volume Fakeout / Vacuum)."
            
            if fvg_bearish or regime_tag == "[NANO]":
                if macro_trend == "BULLISH":
                    return "NEUTRAL", 0.0, f"SMC Tracker {regime_tag}: Bearish Setup blocked by Bullish H4 Trend."
                return f"SELL{signal_suffix}", 0.96, f"SMC {regime_tag}: Bearish Setup (H4 + Volume Verified)"
...
def analyze_market_structure(request: AnalysisRequest, df_macro=None, market_regime="NORMAL") -> AnalysisResponse:
    ...
    # Dead market threshold triggers the NANO tier
